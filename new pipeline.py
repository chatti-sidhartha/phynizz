...
class TryOnConfig:
    ...
    # New accuracy knobs
    shoulder_padding: float = 1.18      # widen garment beyond raw shoulder span
    neck_offset_frac: float = 0.10      # lift garment top toward neck
    feather_radius: int = 25            # mask edge blur (odd number)
    suppress_torso: bool = True         # darken existing clothing before overlay
...
def _extract_pose_points(
    ...
    """Returns (anchors, confidence, segmentation_mask_float32_HxW)."""
    ...
    seg = result.segmentation_mask  # float32 H x W or None
    return anchors, conf, seg
...
def _extract_hmr_points(
    ...
    if all(k in hmr_landmarks for k in keys):
        ...
        anchors["neck"] = ((lsx + rsx) // 2, min(lsy, rsy) - int(0.12 * h))
...
def _garment_mask(garment_img: np.ndarray) -> np.ndarray:
    ...
    light_bg = cv2.inRange(hsv, np.array([0,   0,  190],  dtype=np.uint8),
                                np.array([180, 35, 255],  dtype=np.uint8))
...
def _suppress_existing_torso(
    ...
    pts = np.array([
        ...
        [rhip[0] + pad_x, rhip[1] + pad_y],
        [lhip[0] - pad_x, lhip[1] + pad_y],
    ], dtype=np.int32)
    ...
    if seg_mask is not None:
        person = (seg_mask > 0.5).astype(np.uint8) * 255
        torso  = cv2.bitwise_and(torso, person)

    torso_f = cv2.GaussianBlur(torso.astype(np.float32), (61, 61), 0) / 255.0
    suppress = np.array([25, 25, 25], dtype=np.float32)
...
def _perspective_warp_garment(
    ...
    src = np.float32([
        [x1 + 0.08 * gw, y1 + 0.04 * gh],
        [x1 + 0.92 * gw, y1 + 0.04 * gh],
        [x1 + 0.92 * gw, y1 + 0.98 * gh],
        [x1 + 0.08 * gw, y1 + 0.98 * gh],
    ])
    ...
    dst_ls   = sm - dir * (sw / 2.0) + lift + voff
    dst_rs   = sm + dir * (sw / 2.0) + lift + voff
    tl       = max(40.0, float(np.linalg.norm(((lhip + rhip) / 2.0) - sm)))
    dst_lhip = lhip + np.array([-sw * 0.05, tl * 0.06]) + voff
    dst_rhip = rhip + np.array([ sw * 0.05, tl * 0.06]) + voff
...
def _dense_mesh_warp(
    ...
    sw  = max(1.0, float(np.linalg.norm(rs - ls)))
    pad = sw * config.shoulder_padding
    dir = (rs - ls) / sw
    sm  = (ls + rs) / 2.0
    lift = (neck - sm) * config.neck_offset_frac
    tl   = max(40.0, float(np.linalg.norm(((lhip + rhip) / 2.0) - sm)))

    dst_ls   = sm - dir * (pad / 2.0) + lift + voff
    dst_rs   = sm + dir * (pad / 2.0) + lift + voff
    dst_lhip = lhip + np.array([-sw * 0.05, tl * 0.06]) + voff
    dst_rhip = rhip + np.array([ sw * 0.05, tl * 0.06]) + voff
    ...
    for simplex in tri.simplices:
        ...
        sel = tri_m > 0
        output[dy:dy+dh2, dx:dx+dw2][sel]      = wp[sel]
        output_mask[dy:dy+dh2, dx:dx+dw2][sel] = 255
...
def _feather_mask(mask: np.ndarray, radius: int) -> np.ndarray:
    ...
    interior = cv2.erode(mask, np.ones((max(1, r // 3), max(1, r // 3)), np.uint8))
